...
func TestAnthropicProvider_NewProvider_NoAPIKey(t *testing.T) {
	_, err := NewProvider("")
	require.Error(t, err)
...
func TestAnthropicProvider_Stream_Integration(t *testing.T) {
	// Skip if no API key is set
	apiKey := getTestAPIKey()
	if apiKey == "" {
		t.Skip("ANTHROPIC_API_KEY not set, skipping integration test")
...
// getTestAPIKey retrieves the Anthropic API key from environment
// This is for integration tests only
func getTestAPIKey() string {
	// In real tests, you'd use os.Getenv("ANTHROPIC_API_KEY")
